...
class Leads_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            if (isset($droppable_content)) {
                handle_advance_custom_fields_post($insert_id, $droppable_content,0,"lead","lead_");
            }

            $this->db->select('*');
            $this->db->where('name', 'new_account_leads');
            ...
            if ($map_rule) {
                ...
                if ($acc_accounts) {
                    $this->db->select('*');
                    $this->db->where('key', 'new_account_leads');
                    $query = $this->db->get(db_prefix() . 'mapping_rules')->result_array();

                    if ($query) {
                        $ruleValue = $query[0]['value'];

                        // Check if the rule value matches the next 'number' value
                        if ($ruleValue == $acc_accounts[0]['number'] + 1) {
                            $lastNumber = $ruleValue + 1;
                        } else {
                            $lastNumber = $acc_accounts[0]['number'] + 1;
                        }
                        ...
                        if (get_account_by_name($arr_insert['name'])) {
                            ...
                            // Update the mapping rule with the new 'number' value
                            $this->db->where('key', 'new_account_leads');
                            $this->db->update(db_prefix() . 'mapping_rules', array("value" => $lastNumber));
                        }
                    }
                }
            }
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['droppable_content'])) {
            $droppable_content = $data['droppable_content'];
            handle_advance_custom_fields_post($id, $droppable_content,1,"lead","lead_");
            unset($data['droppable_content']);
        }
        if (!defined('API')) {
    ...
    //     $this->db->select('mail_conversation.*,mail_outbox.*, mail_outbox.id AS mail_outbox_id, mail_conversation.id AS id');
    //     $this->db->from(db_prefix() . 'mail_conversation');
    //     $this->db->join(db_prefix() . 'mail_outbox', 'mail_outbox.id = mail_conversation.outbox_id');
    ...
    foreach ($result as &$conversation) {
        $conversation['attachments'] = $this->get_mail_attachment($conversation['mail_outbox_id'], 'outbox');
    }
