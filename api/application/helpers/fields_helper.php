...
function render_select_nested($name, $options, $option_attrs = [], $label = '', $selected = '', $select_attrs = [], $form_group_attr = [], $form_group_class = '', $select_class = '', $include_blank = true)
{
    $callback_translate = '';
    if (isset($options['callback_translate'])) {
        $callback_translate = $options['callback_translate'];
    ...
    if (!isset($select_attrs['data-none-selected-text'])) {
        $select_attrs['data-none-selected-text'] = _l('dropdown_non_selected_text');
    }
    foreach ($select_attrs as $key => $val) {
        // tooltips
        if ($key == 'title') {
    ...
    $select .= '<select onchange="change_val(this.value)" id="' . $name . '" name="' . $name . '" class="selectpicker' . $select_class . '" ' . $_select_attrs . ' data-live-search="true">';
    if ($include_blank == true) {
        $select .= '<option value=""></option>';
    }

    foreach ($options as $optgroup) {
        if (isset($optgroup['name']) && !empty($optgroup['name']) && isset($optgroup['options']) && is_array($optgroup['options'])) {
            $select .= '<optgroup label="' . $optgroup['name'] . '">';
