...
// zeeshan 
function handle_advance_custom_fields_post($postID = 0, $data="", $updateRequest=0, $fieldto="", $name_slug=""){
    $CI= & get_instance();
    if($updateRequest == 0)
    {
        ...
        if($data != "")
        {
            $CI->db->insert(db_prefix() . 'advance_custom_fields', [
                'fieldto' => $fieldto,
                'rel_id'   => $postID,
                'name' => $name_slug.$postID,
                'slug' => $name_slug.$postID,
                'data'   => $data,
                'active'   => 1,
            ]);
        }
        ...
        if($data != "")
        {
             $CI->db->where('rel_id', $postID);
             $CI->db->where('fieldto', $fieldto);
                $CI->db->update(db_prefix() . 'advance_custom_fields', [
                    'data' => $data,
                ]);
...
function get_advance_custom_fields_html($field_to, $where = [], $exclude_only_admin = false)
{
    $dropableHTML = get_advance_custom_fields($field_to, $where);

    if (is_array($dropableHTML) && isset($dropableHTML[0]['data'])) {
        $dropableHTML = str_replace("&lt;", "<", $dropableHTML[0]['data']);
        $dropableHTML = str_replace("&gt;", ">", $dropableHTML);
    }

    $dataHtml = '';
    $dataHtml .= '<div class="deleted_area "><i class="fa fa-trash"></i></div>';
    $dataHtml .= '<textarea id="droppable_content" name="droppable_content"></textarea>';
    $dataHtml .= '<h3 class="droppableArea text-center">Drag & Drop Custom Fields&nbsp;&nbsp;<span class=" fa fa-arrow-down"> </span></h3>';
    $dataHtml .= '<div class="row dropp_able">';
    
    if ($dropableHTML) {
        $dataHtml .= $dropableHTML;
    }
...
// function get_advance_custom_fields_html($field_to, $where = [], $exclude_only_admin = false)
// {
//     // if (get_advance_custom_fields($field_to, $where)) {
//     $dropableHTML = get_advance_custom_fields($field_to, $where);
//     if ($dropableHTML) {
//         $dropableHTML = str_replace("&lt;", "<", $dropableHTML[0]['data']);
//         $dropableHTML = str_replace("&gt;", ">", $dropableHTML[0]['data']);
...
// // }
// }
function get_advance_custom_fields($field_to, $where = [], $exclude_only_admin = false)
{
    $is_admin = is_admin();
    $CI       = & get_instance();
    $CI->db->where('fieldto', $field_to);
    ...
    $CI->db->where('rel_id', $where);
    $CI->db->where('active', 1);
    $CI->db->order_by('id', 'asc');

    $results = $CI->db->get(db_prefix() . 'advance_custom_fields')->result_array();
