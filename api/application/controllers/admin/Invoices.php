...
class Invoices extends AdminController
{
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            unset($invoice_data['proposal_number_custom']);
            unset($invoice_data['proposal_textarea_custom']);
            unset($invoice_data['proposal_dropdown_custom']);
            unset($invoice_data['proposal_checkbox_custom']);
            unset($invoice_data['proposal_radio_custom']);
            unset($invoice_data['proposal_date_custom']);
            unset($invoice_data['proposal_file_custom']);
            if ($id == '') {
                if (!has_permission('invoices', '', 'create')) {
                    access_denied('invoices');
                }
    ...
    public function get_invoice_data_ajax($id)
    {
        ...
        $data['documnet'] = $this->invoices_model->get_invoice_document($id);
        $data['qr'] = $this->generate_qrcode("https://app.graphiteartistries.com/Fileupload?id=".$id);
