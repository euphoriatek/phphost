...
class Leads extends AdminController
{
    ...
    private function _get_lead_data($id = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            $data['conversation']  = $this->leads_model->get_lead_conversation($id);
    ...
    public function delete_mail_conversation(){
        if ($this->input->post()) {
           $result =  $this->leads_model->delete_mail_conversation($this->input->post('id'));
           if ($result) {
              echo json_encode(['data' => $result, 'message' => "delete_successfully"]);die();
           }else{
