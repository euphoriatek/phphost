                    ...
                    <li class="dd-item dd3-item main<?php echo(!isset($item['collapse']) ? ' dd-nochildren' : ''); ?>"
                        ...
                        <div class="dd3-content"><?php echo _l($item['name'], '', false); ?>
                            <a href="#" class="text-muted toggle-menu-options main-item-options pull-right"><i
                                    class="fa fa-cog"></i></a>
                            <a href="#" class="text-muted pull-right mright5" data-toggle="tooltip" data-placement="right" title="<?php echo $item['tooltip'] ?? "" ?>"><i class="fa-regular fa-circle-question"></i></a>
                        </div>
                        <div class="menu-options main-item-options" style="display:none;"
                            ...
                            <div class="mtop5">
                                <label class="control-label"><?php echo _l('tooltip'); ?></label>
                                <textarea id="tooltip-<?php echo $item['slug']; ?>" cols="30" rows="10" class="form-control"><?php echo (!empty($item['tooltip'])) ? trim($item['tooltip']) : "" ?></textarea>
                            </div>
                        </div>
                        <?php if ($item['slug'] == 'settings') {
                            $settingsTabsValues = moveSettingsTabToSidebarMenu(true);
                            $item['children'] = $settingsTabsValues;
                        } ?>
                        <?php if (count($item['children']) > 0) { ?>
                        <ol class="dd-list dd-list-sub-items">
                            <?php foreach ($item['children'] as $submenu) {
                                      $child_disabled = (isset($menu_options->{$item['slug']}->children->{$submenu['slug']}) && ($menu_options->{$item['slug']}->children->{$submenu['slug']}->disabled == 'true' || $menu_options->{$item['slug']}->children->{$submenu['slug']}->deactivated == 'true')); ?>
                            <li class="dd-item dd3-item sub-items" data-id="<?php echo $submenu['slug']; ?>" <?php if ($child_disabled) {
                                ...
                                <div class="dd3-content"><?php echo _l($submenu['name'], '', false); ?>
                                    <a href="#" class="text-muted toggle-menu-options sub-item-options pull-right"><i
                                                class="fa fa-cog"></i></a>
                                    <a href="#" class="text-muted pull-right mright5" data-toggle="tooltip" data-placement="right" title="<?php echo $submenu['tooltip'] ?? "" ?>"><i class="fa-regular fa-circle-question"></i></a>
                                </div>
                                <div class="menu-options sub-item-options" style="display:none;"
                                    data-menu-options="<?php echo $submenu['slug']; ?>">
                                    <?php if ($submenu['parent_slug'] == 'settings'): ?>
                                        <div class="checkbox">
                                            <input type="checkbox" id="deactivated-<?php echo $submenu['slug']; ?>" <?php if ($child_disabled) {
                                              echo ' checked';
                                          } ?>>
                                            <label for="deactivated-<?php echo $submenu['slug']; ?>">Disabled?</label>
                                        </div>
                                    <?php else: ?>
                                    ...
                                    <div class="mtop5">
                                        <label class="control-label"><?php echo _l('tooltip'); ?></label>
                                        <textarea id="tooltip-<?php echo $submenu['slug']; ?>" cols="30" rows="10" class="form-control"><?php echo !empty($submenu['tooltip']) ? trim($submenu['tooltip']) : ""; ?></textarea>
                                    </div>
                                </div>
                            </li>
                            <?php
...
function save_menu() {
    ...
    $.each(items, function(key, val) {
        ...
        main_menu.data('tooltip', main_menu.find('#tooltip-' + main_menu.data('id')).val());
        ...
        $.each(sub_items, function(subKey, val) {
            ...
            sub_item.data('tooltip', sub_item.find('#tooltip-' + sub_item.data('id')).val());
            sub_item.data('deactivated', sub_item.find('#deactivated-' + sub_item.data('id')).prop('checked') === true);
