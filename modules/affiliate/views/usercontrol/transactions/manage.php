        ...
        <table class="table dt-table table-transactions">
            ...
            <tbody>
              <?php foreach($transactions as $transaction){ ?>
                <tr>
                  <td>
                    <div class="checkbox"><input type="checkbox" <?php if($transaction['status'] != 0){ echo 'disabled'; }?> value="<?php echo html_entity_decode($transaction['id']); ?>" data-amount="<?php echo html_entity_decode($transaction['amount']); ?>"><label></label></div></td>
                  <td><?php echo _d($transaction['datecreated']); ?></td>
                  <td><?php echo html_entity_decode($transaction['username']); ?></td>
                  <td><?php echo html_entity_decode($transaction['order_id']); ?></td>
                  ...
                  if($transaction['status'] == 1){
                   ...
                   ?>
                  <td><span class="label label-<?php echo html_entity_decode($label_class); ?> s-status commission-status-<?php echo html_entity_decode($transaction['status']); ?>"><?php echo html_entity_decode($status_name); ?></span></td>
                </tr>
             <?php } ?>
            </tbody>
         </table> 
