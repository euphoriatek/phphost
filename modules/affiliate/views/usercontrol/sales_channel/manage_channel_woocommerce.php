		...
		<table class="table dt-table">
            ...
            <tbody>
              <?php foreach($channels as $channel){ ?>
                <tr>
                  	<td><a href="<?php echo site_url('affiliate/usercontrol/woocommerce_channel_detail/'.$channel['id']); ?>"><?php echo html_entity_decode($channel['name_channel']); ?></a></td>
                  	<td><?php echo html_entity_decode($channel['url']); ?></td>
                  	<td><?php echo html_entity_decode($channel['consumer_key']); ?></td>
                  	<td><?php echo html_entity_decode($channel['consumer_secret']); ?></td>
                    <td><?php echo icon_btn(site_url('affiliate/usercontrol/woocommerce_channel_detail/'.$channel['id']), 'eye', 'btn-default', [
                        'title' => _l('view')
                    ]).''.icon_btn('#', 'edit', 'btn-default', [
                        'title' => _l('edit'),
                        'data-id' => html_entity_decode($channel['id']),
                        'data-name' => html_entity_decode($channel['name_channel']),
                        'data-key' => html_entity_decode($channel['consumer_key']),
                        'data-secret' => html_entity_decode($channel['consumer_secret']),
                        'data-url' => html_entity_decode($channel['url']),
                        'onclick' => 'edit(this); return false;'
                    ]).''.icon_btn(site_url('affiliate/usercontrol/delete_woocommerce_channel/'.$channel['id']), 'remove', 'btn-danger _delete', [
                        'title' => _l('delete') 
                    ]); ?></td>
                </tr>
             <?php } ?>
