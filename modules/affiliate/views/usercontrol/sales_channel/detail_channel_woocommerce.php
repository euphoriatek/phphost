...
<table class="table dt-table table-product-woocommerce">
  ...
  <tbody>
    <?php foreach($list_product as $product){ ?>
      <tr>
          <td>
            <div class="checkbox"><input type="checkbox" value="<?php echo html_entity_decode($product['product_id']); ?>"><label></label></div>
          </td>
          <td><a href="javascript:void(0)" onclick="product_detail(<?php echo html_entity_decode($product['product_id']); ?>, ''); return false;"><?php echo html_entity_decode($product['commodity_code']); ?></a></td>
          <td><?php echo html_entity_decode($product['description']); ?></td>
          <td><?php echo html_entity_decode($product['rate']); ?></td>
          <td><?php echo html_entity_decode($product['prices']); ?></td>
          <td><?php echo icon_btn('#', 'eye', 'btn-default', [
              'title' => _l('view'),
              'onclick' => 'product_detail('.$product['product_id'].'); return false;',
          ]).''.icon_btn('#', 'edit', 'btn-default', [
              'title' => _l('edit'),
          ...
          ]).''.icon_btn(site_url('affiliate/usercontrol/delete_product_store/'.$product['woocommere_product_id'].'/'.$product['woocommere_channel_id']), 'remove', 'btn-danger _delete', [
              'title' => _l('delete') 
          ]); ?></td>
      </tr>
   <?php } ?>
...
<div class="modal-dialog">
    ...
    <?php echo form_open(site_url('affiliate/usercontrol/add_product_channel_wcm'),array('id'=>'form_add_product')); ?>             
        <div class="modal-body">
           <div class="row">
            <input type="hidden" name="woocommere_channel_id" value="<?php echo html_entity_decode($id); ?>">
                ...
                <label for="group_product_id" class="control-label"><?php echo _l('group_product'); ?></label>
                  <select id="group_product_id" name="group_product_id" class="selectpicker" data-width="100%" data-none-selected-text="Nothing selected" data-live-search="true" data-actions-box="true" tabindex="-98" onchange="get_list_product(this); return false;">
                    ...
                    <?php foreach ($group_product as $key => $value){ ?>
                      <option value="<?php echo html_entity_decode($value['id']); ?>"><?php echo html_entity_decode($value['commodity_group_code'].' # '.$value['name']); ?></option>
                    <?php } ?>
                  </select>
             </div>
            </div>
                ...
                <label for="product_id" class="control-label"><?php echo _l('affiliate_product'); ?></label>
                  <select id="product_id" name="product_id[]" class="selectpicker" multiple  data-width="100%" data-none-selected-text="Nothing selected" data-live-search="true" data-actions-box="true" tabindex="-98">
                    <?php foreach ($products as $key => $value){ ?>
                      <option value="<?php echo html_entity_decode($value['id']); ?>"><?php echo html_entity_decode($value['commodity_code'].' # '.$value['description']); ?></option>
                    <?php } ?>
                  </select>
             </div>
            </div>
