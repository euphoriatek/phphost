            ...
            <tbody>
               <?php foreach($product_list as $product){ ?>
                <tr>
                  ...
                  <td><?php echo html_entity_decode($product['sku_code']); ?></td>
                  ...
                  <td><?php echo icon_btn(site_url('affiliate/usercontrol/delete_product/'.$product['user_product_id']), 'remove', 'btn-danger _delete', [
                        'title' => _l('delete') 
                    ]); ?></td>
                </tr>
             <?php } ?>
                        ...
                        <div class="form-group" id="report-time">
                          ...
                          <select class="selectpicker" name="product[]" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-actions-box="true" multiple="true" data-live-search="true">
                              <?php foreach ($products as $key => $value) { ?>
                                  <option value="<?php echo html_entity_decode($value['id']); ?>"><?php echo html_entity_decode($value['label']); ?></option>
                              <?php } ?>
                          </select>
                        </div>
                    </div>
