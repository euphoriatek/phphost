...
<tbody>
  <?php foreach($setting_woo_store as $setting){ 
  	...
  	?>
    <tr>
      <td><?php echo html_entity_decode($setting['name_channel']); ?></td>
      <td><?php echo html_entity_decode($time1); ?></td>
      <td><?php echo html_entity_decode($time2); ?></td>
      <td><?php echo html_entity_decode($time3); ?></td>
      <td><?php echo html_entity_decode($time4); ?></td>
      <td><?php echo html_entity_decode($time5); ?></td>
      <td><?php echo html_entity_decode($time6); ?></td>
      <td><?php echo icon_btn('#', 'edit', 'btn-default', [
                        'title' => _l('edit'),
                        'data-id' => html_entity_decode($setting['id']),
                        'data-store' => html_entity_decode($setting['store']),
                        'data-sync_omni_sales_products' => html_entity_decode($setting['sync_omni_sales_products']),
                        'data-sync_omni_sales_inventorys' => html_entity_decode($setting['sync_omni_sales_inventorys']),
                        'data-price_crm_woo' => html_entity_decode($setting['price_crm_woo']),
                        'data-sync_omni_sales_description' => html_entity_decode($setting['sync_omni_sales_description']),
                        'data-sync_omni_sales_images' => html_entity_decode($setting['sync_omni_sales_images']),
                        'data-sync_omni_sales_orders' => html_entity_decode($setting['sync_omni_sales_orders']),
                        'data-time1' => html_entity_decode($setting['time1']),
                        'data-time2' => html_entity_decode($setting['time2']),
                        'data-time3' => html_entity_decode($setting['time3']),
                        'data-time4' => html_entity_decode($setting['time4']),
                        'data-time5' => html_entity_decode($setting['time5']),
                        'data-time6' => html_entity_decode($setting['time6']),
                        'onclick' => 'update_setting_woo_store(this); return false;'
                    ]).''.icon_btn(site_url('affiliate/usercontrol/delete_sync_auto_store/'.$setting['id']), 'remove', 'btn-danger _delete', [
                        'title' => _l('delete') 
                    ]); ?></td>
    </tr>
 <?php } ?>
                ...
                <label for="store" class="control-label"><?php echo _l('store'); ?></label>
                  <select id="store" name="store" class="selectpicker" data-width="100%" data-none-selected-text="Nothing selected" data-live-search="true" data-actions-box="true" tabindex="-98">
                    ...
                    <?php foreach ($store as $key => $value){ ?>
                      <option value="<?php echo html_entity_decode($value['id']); ?>"><?php echo html_entity_decode($value['name_channel']); ?></option>
                    <?php } ?>
                  </select>
             </div>
            </div>
