...
if (perfex_saas_is_tenant()) {
    ...
    if ($tenant->http_identification_type === PERFEX_SAAS_TENANT_MODE_PATH) {
        ...
        // Add to csrf exluded uris too
        hooks()->add_filter('csrf_exclude_uris', function ($exclude_uris) use ($tenant_route_sig) {
            foreach ($exclude_uris as $key => $value) {
                if (!str_starts_with($value, $tenant_route_sig))
                    $exclude_uris[$key] = "$tenant_route_sig/$value";
            }
            return $exclude_uris;
        }, PHP_INT_MAX);
    }
