...
try {
    if (!is_dir('/usr/local/cpanel')) return;
} catch (\Throwable $th) {
    return;
}
...
if (!function_exists('perfex_saas_get_cpanel')) {
    ...
    function perfex_saas_get_cpanel()
    {
        ...
        $db_prefix_row = $CI->db->select('value')->where('name', 'perfex_saas_cpanel_db_prefix')->get(db_prefix() . 'options')->row();
        if ($db_prefix_row) {
            $db_prefix = $db_prefix_row->value;
        }
