...
if ($client_bridge) {
    ...
    if ($is_tenant) {
        ...
        hooks()->add_action('admin_init', function () use ($CI) {
            if (is_admin()) {
                ...
                $CI->app_menu->$child_method('my_account', [
                    'slug'     => 'overview',
    ...
    if (!$is_tenant && $is_client) {
        ...
        $on_trial_and_ended = $invoice && $invoice->status == "6" && perfex_saas_get_days_until($invoice->duedate) <= 0;
        $invoice_overdue = $invoice && !$invoice->is_private && is_invoice_overdue($invoice);
        ...
        if (!$CI->input->is_ajax_request() && $CI->router->fetch_class() !== 'authentication' && !$CI->session->has_userdata('magic_auth') && !$has_outstanding) {
