...
function perfex_saas_deployer($company_id = '', $clientid = '', $limit = 5)
{
    ...
    foreach ($pending_companies as $company) {
        try {
            ...
            $total_deployed += 1;
            $cron_urls[] = perfex_saas_tenant_base_url($company, 'cron', 'path');
        } catch (\Exception $e) {
...
function perfex_saas_deploy_company($company)
{
    ...
    try {
        ...
        $url = perfex_saas_tenant_base_url($company, 'cron', 'path');
        perfex_saas_http_request($url, ['timeout' => 10]);
...
function perfex_saas_master_default_tables($folder_path = '')
{
    ...
    $folder_path = empty($folder_path) ? APP_MODULES_PATH . PERFEX_SAAS_MODULE_NAME . '/migrations/default_seeds' : $folder_path;
 ...
 * @return array
 */
function perfex_saas_seed_tables()
{
    $dbprefix = perfex_saas_master_db_prefix();
    $default_table_selectors = perfex_saas_default_seed_tables();

    $seed_tables = get_option('perfex_saas_tenants_seed_tables');
    $seed_tables = empty($seed_tables) ?
        array_keys($default_table_selectors) :
        array_merge((array)json_decode($seed_tables), [$dbprefix . 'options', $dbprefix . 'migrations']);
...
function perfex_saas_setup_seed($company, $dsn)
{
    ...
    $default_table_selectors = perfex_saas_default_seed_tables();
    $seed_tables = perfex_saas_seed_tables();
...
function perfex_saas_clear_sensitive_data($company, $dsn_array, $invoice = null)
{
    ...
    // Reset nex invoice, estimate, creditnote number
...
function perfex_saas_setup_modules_for_tenant($verbose = true)
{
    ...
    foreach ($core_required_default_modules as $module) {
        ...
        if ($verbose)
            echo "Installing core module ($module) - required for perfect running of the CRM";
...
{
    return perfex_saas_get_options('perfex_saas_enable_single_package_mode') === '1';
}
...
{
    return in_array($company->status, ['pending', 'deploying']);
}
