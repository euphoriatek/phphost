...
if (!$CI->db->field_exists('settlement' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `settlement` VARCHAR(200) NULL;');
}

if (!$CI->db->field_exists('approved' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `approved` VARCHAR(200) NULL;');
}

if (!$CI->db->field_exists('line' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `line` VARCHAR(200) NULL;');
}

if (!$CI->db->field_exists('document' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `document` VARCHAR(200) NULL;');
}

if (!$CI->db->field_exists('account_name' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `account_name` VARCHAR(200) NULL;');
}

if (!$CI->db->field_exists('balance' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `balance` VARCHAR(200) NULL;');
}

if (!$CI->db->field_exists('account_number' ,db_prefix() . 'acc_account_history')) {
  $CI->db->query('ALTER TABLE `' . db_prefix() . 'acc_account_history`
    ADD COLUMN `account_number` INT(11) NULL;');
}
...
if (!$CI->db->table_exists(db_prefix() . 'unified_book_mapping')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'unified_book_mapping` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_prefix() . 'unified_book')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'unified_book` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
...
if (!$CI->db->table_exists(db_prefix() . 'acc_account_types')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'acc_account_types` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
        `name` VARCHAR(200),
        `order` int(11),
        `datesend` VARCHAR(200) DEFAULT NULL,
      PRIMARY KEY (`id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=' . $CI->db->char_set . ';');
}
