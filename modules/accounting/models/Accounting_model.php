...
class Accounting_model extends App_Model
{
    ...
    {
        $this->db->select('*')->from(db_prefix().'acc_account_types');
        $query = $this->db->get();
        $result = $query->result();
        $newArray = [];
        ...
        $account_types = hooks()->apply_filters('before_get_account_types', $newArray);
    ...
    public function update_automatic_conversion($data){
        ...
        if(!isset($data['acc_account_relations'])){
            $data['acc_account_relations'] = 0;
        }
        if (isset($data['choose_module'])) {
            $this->db->where('name', $data['choose_module']);
            $result = $this->db->get(db_prefix() . 'options')->row();
            if($result){
                $this->db->where('name', $data['choose_module']);
                // $this->db->get(db_prefix() . 'options')->row();
                $this->db->update(db_prefix() . 'options', [
                    'value' => $data['chart_of_accounts'],
                    'autoload' => 1
                ]);
                if ($this->db->affected_rows() > 0) {
                    $affectedRows++;
                ...
                $node = [];
                $node['name'] = $data['choose_module'];
                $node['value'] = $data['chart_of_accounts'];
                $node['autoload'] = 1;
                $this->db->insert(db_prefix().'options', $node);
                $reconcile_id = $this->db->insert_id();
                if ($reconcile_id) {
                   $affectedRows++;
                }
            }
        }else if(isset($data['document_module'])){
            $sql = "SELECT * FROM " . db_prefix() . "options 
            WHERE name = ? 
            AND JSON_UNQUOTE(JSON_EXTRACT(value, '$.relation_variable')) = ?";
            $query = $this->db->query($sql, array($data['document_module'], $data['relation_variable']));
            $result = $query->row();
            // $this->db->where('name', $data['document_module']);
            // $result = $this->db->get(db_prefix() . 'options')->row();
            $this->db->select('name');
            $this->db->where('id', $data['chart_of_accounts']);
            $query = $this->db->get(db_prefix().'acc_accounts')->result_array();
            if($result){
                // $this->db->where('name', $data['document_module']);
                // $this->db->where('JSON_UNQUOTE(JSON_EXTRACT(value, "$.relation_variable"))', $data['relation_variable']);
                // // $this->db->get(db_prefix() . 'options')->row();
                // $this->db->update(db_prefix() . 'options', [
                //     'value' => json_encode(array(
                //                         "relation_variable" => $data['relation_variable'],
                ...
                $node = [];
                $node['name'] = $data['document_module'];
                $node['value'] = json_encode(array(
                                        "relation_variable" => $data['relation_variable'],
                                        "chart_of_accounts" => $data['chart_of_accounts'],
                                        "action_name" => $data['action_name'],
                                        "account_name" =>$query[0]['name']
     ...
     * @return     boolean 
     */
    public function update_unified_book($data){
        $affectedRows = 0;
        $this->load->dbforge();
        $existingFields = $this->db->list_fields(db_prefix() . 'unified_book');
        if (!in_array($data['relation_variable'], $existingFields)) {
        $fields = array(
            $data['relation_variable'] => array(
                'type' => 'VARCHAR',
                'constraint' => 100,
                'null' => true,
            ),
        ...
        if ($this->dbforge->add_column('unified_book', $fields)) {
            $this->db->where('name', $data['document_module']);
            $result = $this->db->get(db_prefix() . 'options')->row();
            if($result){
                $this->db->where('name', $data['document_module']);
                $this->db->update(db_prefix() . 'options', [
                    'value' => $data['relation_variable'],
                    'autoload' => 1
                ]);
            }else{
                $node = [];
                $node['name'] = $data['document_module'];
                $node['value'] = $data['relation_variable'];
                $node['autoload'] = 1;
                $this->db->insert(db_prefix().'options', $node);
                $reconcile_id = $this->db->insert_id();
            }
        ...
        $this->db->where('value', $data['relation_variable']);
        $getdata = $this->db->get(db_prefix() . 'unified_book_mapping')->row();
        if ($getdata) {
           $this->db->where('value', $data['relation_variable']);
            $this->db->update(db_prefix() . 'unified_book_mapping', ["row_number" => $data['row_number']]);
        }else{
            $mapping_array = array(
                "key_name" => $data['document_module'],
                "value" => $data['relation_variable'],
                "row_number" => $data['row_number']
            );
            $this->db->insert(db_prefix().'unified_book_mapping', $mapping_array);
        }
        if ($affectedRows > 0) {
            return true;
        }
    ...
    public function add_account($data)
    {
        ...
        if(isset($data['update_balance'])){
       	...
       	if (isset($data['droppable_content'])) {
            $droppable_content = $data['droppable_content'];
            unset($data['droppable_content']);
        }
        ...
        if ($insert_id) {
          	...
          	if(isset($droppable_content)) {
                handle_advance_custom_fields_post($insert_id, $droppable_content,0,"chart_of_accounts","chart_of_accounts_");
            }
     ...
     * @return     array  The product group select.
     */
    public function get_data_account_to_select_number() {
        ...
        foreach ($accounts as $key => $account) {
            ...
            $note['id'] = $account['number'];
            
            $note['label'] = $account['number'];
    ...
    public function add_journal_entry($data){
        ...
        if($insert_id){
            $data_insert = [];
			if (isset($droppable_content)) {
                handle_advance_custom_fields_post($insert_id, $droppable_content,0,"journal_entry","journal_entry_");
            }
            foreach ($journal_entry as $key => $value) {
                if($value[0] != ''){
                    ...
                    $node['debit'] = $value[1];
                    $node['credit'] = $value[2];
                    $node['account_number'] = $value[3];
                    $node['description'] = $value[4];
                    $node['rel_id'] = $insert_id;
                    $node['rel_type'] = 'journal_entry';
                    $node['datecreated'] = date('Y-m-d H:i:s');
                    $node['addedfrom'] = get_staff_user_id();
                    $node['settlement'] = $value[5];
                    $node['line'] = $value[6];
                    // $node['rel_type'] = $value[7];
                    $node['document'] = $value[8];
                    $node['balance'] = $value[9];
                    $this->db->where('id', $value[0]);
                    $acc_accounts = $this->db->get(db_prefix() . 'acc_accounts')->result_array();
                    if ($acc_accounts) {
                        $node['account_name'] = $acc_accounts[0]['name'];
                    }
                    $data_insert[] = $node;
                }
            }
    ...
    public function get_unified_entry($id){
        $this->db->where('id', $id);
        $unified_book = $this->db->get(db_prefix() . 'unified_book')->row();
        return $unified_book;
    }
    ...
    public function update_journal_entry($data, $id){
		...
		if (isset($data['droppable_content'])) {
            $droppable_content = $data['droppable_content'];
            handle_advance_custom_fields_post($id, $droppable_content,1,"journal_entry","journal_entry_");
            unset($data['droppable_content']);
        }
        $this->db->where('id', $id);
    ...
    public function automatic_invoice_conversion($invoice_id){
        ...
        if($invoice){
            ...
            $this->db->select('*');
            $this->db->where('name', 'document_invoices');
            $this->db->where('autoload', 1);
            $map_rule = $this->db->get(db_prefix() . 'options')->result_array();
            if ($map_rule) {
                for($i=0;$i < count($map_rule); $i++ ){
                $this->db->select('*');
                $this->db->where('id', json_decode($map_rule[$i]['value'], true)['chart_of_accounts']);
                $acc_accounts = $this->db->get(db_prefix() . 'acc_accounts')->result_array();
                if ($acc_accounts) {
                        $data = array(
                            "journal_date" => date("Y-m-d"),
                            "number" => 1,
                            "journal_entry" => [[json_decode($map_rule[$i]['value'], true)['chart_of_accounts'],$item_total,"","Entry For When Create New Invoice"]],
                            "amount" => $item_total,
                            "description" => "Create New Invoice"
                        );
                        if (get_account_by_name($acc_accounts[0]['name'])) {
                            ...
                            if($insert_id){
                                ...
                                foreach ($journal_entry as $key => $value) {
                                    if($value[0] != ''){
                                        ...
                                        $node['settlement'] = rand();
                                        $node['document'] = 'invoice';
                                        $node['line'] = 2;
                                        $node['balance'] = $item_total;
                                        $node['approved'] = 'yes';
                                        $node['account_name'] = $acc_accounts[0]['name'];
                                        $node['rel_id'] = $insert_id;
                                        $node['rel_type'] = 'journal_entry';
            ...
            $this->db->select('*');
            $this->db->where('name', 'unified_invoices');
            $this->db->where('autoload', 1);
            $unified_map_rule = $this->db->get(db_prefix() . 'options')->result_array();
            if ($unified_map_rule) {
                // $query = $this->db->query("SHOW COLUMNS FROM ".db_prefix()."unified_book LIKE '%invoices'");
                $query = $this->db->query("SELECT COLUMN_NAME
                            FROM INFORMATION_SCHEMA.COLUMNS
                            WHERE TABLE_NAME = 'tblunified_book'
                              AND (COLUMN_NAME LIKE '%general%' OR COLUMN_NAME LIKE '%company%' OR COLUMN_NAME LIKE '%customers%' OR COLUMN_NAME LIKE '%invoices%')");
                $columns = $query->result_array();
                $resultArray = [];
            $this->db->where('id', $invoice_id);
            $invoice_data = $this->db->get(db_prefix() . 'invoices')->result_array();
            foreach ($columns as $column) {
                $temp = explode('_', strrev($column['COLUMN_NAME']), 2);
                $string = strrev($temp[1]);
                if (array_key_exists($string, $invoice_data[0])) {
                    $resultArray[$column['COLUMN_NAME']] = $invoice_data[0][$string];
                }
                $this->db->select('*');
                $this->db->where('name', $string);
                $check_in_option = $this->db->get(db_prefix() . 'options')->result_array();
                if ($check_in_option) {
                    $resultArray[$column['COLUMN_NAME']] = $check_in_option[0]['value'];
                }
            }
            if (count($resultArray) > 0) {
                $this->db->where('id_invoices', $invoice_id);
                $unified_book = $this->db->get(db_prefix() . 'unified_book')->result_array();
                if ($unified_book) {
                    $this->db->where('id_invoices', $invoice_id);
                    $this->db->update(db_prefix() . 'unified_book', $resultArray);
                }else{
                    $this->db->insert(db_prefix() . 'unified_book', $resultArray);
                } 
            }
            }     
            if ($affectedRows > 0) {
        ...
        if($expense){
            ...
            if($data_insert != []){
                $affectedRows = $this->db->insert_batch(db_prefix().'acc_account_history', $data_insert);
                        ...
                        $this->db->select('*');
            $this->db->where('name', 'document_expenses');
            $this->db->where('autoload', 1);
            $map_rule = $this->db->get(db_prefix() . 'options')->result_array();
            if ($map_rule) {
                for($i=0;$i < count($map_rule); $i++ ){
                ...
                if ($acc_accounts) {
                        $data = array(
                            "journal_date" => date("Y-m-d"),
                            "number" => 1,
                            "journal_entry" => [[json_decode($map_rule[$i]['value'], true)['chart_of_accounts'],"",$expense->amount,"Entry For When Create New Expenses"]],
            ...
            $this->db->select('*');
            $this->db->where('name', 'unified_expenses');
            $this->db->where('autoload', 1);
            $unified_map_rule = $this->db->get(db_prefix() . 'options')->result_array();
            if ($unified_map_rule) {
                $query = $this->db->query("SHOW COLUMNS FROM ".db_prefix()."unified_book LIKE '%expenses'");
                $columns = $query->result_array();
                $resultArray = [];
            $this->db->where('id', $expense_id);
            $invoice_data = $this->db->get(db_prefix() . 'expenses')->result_array();
            foreach ($columns as $column) {
                $temp = explode('_', strrev($column['Field']), 2);
                $string = strrev($temp[1]);
                if (array_key_exists($string, $invoice_data[0])) {
                    $resultArray[$column['Field']] = $invoice_data[0][$string];
                }
            }
            if (count($resultArray) > 0) {
                $this->db->where('id_expenses', $expense_id);
                $unified_book = $this->db->get(db_prefix() . 'unified_book')->result_array();
                if ($unified_book) {
                    $this->db->where('id_expenses', $expense_id);
                    $this->db->update(db_prefix() . 'unified_book', $resultArray);
    ...
    public function get_mapping(){
        $array = array('new_account_customers','new_account_contracts','new_account_projects','new_account_tasks','new_account_support','new_account_leads','new_account_taxes','new_account_currency','new_account_payment_mode','new_account_suppliers','new_account_bancs','new_account_products','new_account_immobilitzed','new_account_staff');
        $this->db->select(db_prefix() .'options.*, '.db_prefix() .'acc_accounts.name AS account_name'.','.db_prefix() .'acc_accounts.number');
        $this->db->from(db_prefix() . 'options');
        $this->db->join(db_prefix() . 'acc_accounts', db_prefix() .'options.value = '.db_prefix() .'acc_accounts.id', 'left');
        $this->db->where(db_prefix() . 'options.autoload', 1);
        $this->db->where_in(db_prefix() .'options.name', $array);
        
        $accounts = $this->db->get()->result_array();
        return $accounts;
    }
    
    public function mapping_settlement(){
        $array = array('document_invoices','document_expenses','document_payrolls','document_purchase_order','document_purchase_payment','document_receiving_boucher','document_delivery','document_loss','document_adjustment','document_opening_stock','document_received','document_done');
        $this->db->from(db_prefix() . 'options');
        $this->db->where(db_prefix() . 'options.autoload', 1);
        $this->db->where_in(db_prefix() .'options.name', $array);
    ...
    public function delete_mapping($mapping_id){
        ...
        $affectedRows = $this->db->update(db_prefix().'options',  ['autoload' => 0]);
        if ($affectedRows > 0) {
            return true;
        }else{
            return false;
    ...
    public function get_unified_book(){
        $this->db->select('*');
        $this->db->from(db_prefix() .'unified_book');
        $unified_book = $this->db->get()->result_array();
        return $unified_book;
    }

    public function unified_book_mapping(){
        $this->db->select('*');
        $this->db->from(db_prefix() . 'unified_book_mapping');
        $this->db->where_not_in('value', array('id_invoices', 'id_expenses'));
        $Fields_For_Loop = $this->db->get()->result_array();
        return $Fields_For_Loop;
    }

    /**
     * Automatic conversion for all modules
     * @param  integer $id 
     * @return boolean
     */
    public function automatic_conversion_module($id,$key){
            $this->db->select('*');
            $this->db->where('name', $key);
            $this->db->where('autoload', 1);
            $map_rule = $this->db->get(db_prefix() . 'options')->result_array();
            ...
            if ($map_rule) {
                $this->db->select('*');
                $this->db->where('id', $map_rule[0]['value']);
                $acc_accounts = $this->db->get(db_prefix() . 'acc_accounts')->result_array();

                if ($acc_accounts) {
                        $lastNumber = $acc_accounts[0]['number']."".$id;
                        $arr_insert = array(
                            "account_type_id" => $acc_accounts[0]['account_type_id'],
                            "account_detail_type_id" => $acc_accounts[0]['account_detail_type_id'],
                            "number" => $lastNumber,
                            "name" => $acc_accounts[0]['name'],
                            "parent_account" => $acc_accounts[0]['parent_account'],
                            "active" => $acc_accounts[0]['active']
                        );

                        if (get_account_by_name($arr_insert['name'])) {
                            $this->db->insert(db_prefix() . 'acc_accounts', $arr_insert);
     ...
     /**
     * Drop Columns of unified Mapping
     */
    
  public function delete_unified_mapping($mapping) {
     
        $table_name = db_prefix() . 'unified_book';
        $columns = $this->db->list_fields($table_name);
 
        if (in_array($mapping, $columns)) {
            $query = "ALTER TABLE $table_name DROP COLUMN $mapping";
            
            if ($this->db->query($query)) {
                $this->db->where('value', $mapping);
                $this->db->delete(db_prefix().'unified_book_mapping');
                return true;
            }
        } else {
            return false;
    ...
    public function update_unified_book_data($data, $id){
            $affectedRows = 0;
            $this->db->where('id', $id);
            $this->db->update(db_prefix() . 'unified_book', $data);
            if ($this->db->affected_rows() > 0) {
                $affectedRows++;
            }
            if ($affectedRows > 0) {
