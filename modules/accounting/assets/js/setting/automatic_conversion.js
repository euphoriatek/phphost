...
(function($) {
  ...
  $('input[name="acc_account_relations"]').on('change', function() {
    if($('input[name="acc_account_relations"]').is(':checked') == true){
      $('#div_acc_account_relations').removeClass('hide');
    }else{
      $('#div_acc_account_relations').addClass('hide');
    }
});

  init_item_automatic_table();
  change_val("document_invoices");
...
function change_val(data) {
   if (data) {
       const element_blank = document.getElementById('relation_variable');
       element_blank.innerHTML = '';
        $.ajax({
        url : 'https://'+window.location.host+'/admin/accounting/get_relation_variable',
        type : 'GET',
        data : {'data' : data},
        dataType:'json',
    ...
    const selectElement = document.getElementById('relation_variable').innerHTML = '';
   }
  }