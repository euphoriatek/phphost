...
// Create hook by 11/21/2023
hooks()->add_filter('after_client_created', 'acc_automatic_client_conversion');
hooks()->add_filter('after_contract_added', 'acc_automatic_contract_conversion');
hooks()->add_filter('after_add_project', 'acc_automatic_project_conversion');
hooks()->add_filter('after_add_task', 'acc_automatic_task_conversion');
hooks()->add_filter('ticket_created', 'acc_automatic_ticket_conversion');
hooks()->add_filter('lead_created', 'acc_automatic_lead_conversion');
hooks()->add_filter('after_tax_created', 'acc_automatic_tax_conversion');
hooks()->add_filter('after_currency_created', 'acc_automatic_currency_conversion');
hooks()->add_filter('after_paymentmode_added', 'acc_automatic_paymentmode_conversion');
hooks()->add_filter('staff_member_created', 'acc_automatic_staff_member_conversion');
...
function accounting_load_js() {
	...
	if (!(strpos($viewuri, 'admin/accounting/unified_book?group=unified_book') === false)) {
		echo '<script src="' . module_dir_url(ACCOUNTING_MODULE_NAME, 'assets/js/unified_book/manage.js') . '?v=' . ACCOUNTING_REVISION . '"></script>';
	}
}
	...
	if (has_permission('accounting_dashboard', '', 'view') || has_permission('accounting_transaction', '', 'view') || has_permission('accounting_journal_entry', '', 'view') || has_permission('accounting_transfer', '', 'view') || has_permission('accounting_chart_of_accounts', '', 'view') || has_permission('accounting_reconcile', '', 'view') || has_permission('accounting_report', '', 'view') || has_permission('accounting_setting', '', 'view')) {
		...
		if (has_permission('accounting_journal_entry', '', 'view')) {
			$CI->app_menu->add_sidebar_children_item('accounting', [
				...
				if (has_permission('accounting_journal_entry', '', 'view')) {
			$CI->app_menu->add_sidebar_children_item('accounting', [
				'slug' => 'accounting_unified_book',
				'name' => _l('unified_book'),
				'icon' => 'fa fa-book',
				'href' => admin_url('accounting/unified_book?group=unified_book'),
				'position' => 4,
			]);
		}
...
function acc_automatic_client_conversion($client_id){
	if ($client_id['id']) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($client_id['id'],"new_account_customers");

	}
	return $client_id['id'];
}

function acc_automatic_contract_conversion($contract_id){
	if ($contract_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($contract_id,"new_account_contracts");

	}
	return $contract_id;
}

function acc_automatic_project_conversion($project_id){
	if ($project_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($project_id,"new_account_projects");

	}
	return $project_id;
}

function acc_automatic_task_conversion($task_id){
	if ($task_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($task_id,"new_account_tasks");

	}
	return $task_id;
}

function acc_automatic_ticket_conversion($ticket_id){
	if ($ticket_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($ticket_id,"new_account_support");

	}
	return $ticket_id;
}

function acc_automatic_lead_conversion($lead_id){
	if ($lead_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($lead_id,"new_account_leads");

	}
	return $lead_id;
}

function acc_automatic_tax_conversion($tax_id){
	if ($tax_id['id']) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($tax_id['id'],"new_account_taxes");

	}
	return $tax_id['id'];
}

function acc_automatic_currency_conversion($currency_id){
	if ($currency_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($currency_id,"new_account_currency");

	}
	return $currency_id;
}

function acc_automatic_paymentmode_conversion($payment_id){
	if ($payment_id['id']) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($payment_id['id'],"new_account_payment_mode");

	}
	return $payment_id['id'];
}

function acc_automatic_staff_member_conversion($staff_id){
	if ($staff_id) {
			$CI = &get_instance();
			$CI->load->model('accounting/accounting_model');
			$CI->accounting_model->automatic_conversion_module($staff_id,"new_account_staff");
