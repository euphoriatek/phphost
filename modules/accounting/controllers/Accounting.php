...
class Accounting extends AdminController
{
    ...
    public function setting()
    {
        ...
        $data['module'] = array(
            array('id' => 1, "name" => "Customers" , "key" => "new_account_customers"),
            array('id' => 2, "name" => "Contracts", "key" => "new_account_contracts"),
            array('id' => 3, "name" => "Projects", "key" => "new_account_projects"),
            array('id' => 4, "name" => "Tasks", "key" => "new_account_tasks"),
            array('id' => 5, "name" => "Support", "key" => "new_account_support"),
            array('id' => 6, "name" => "Leads", "key" => "new_account_leads"),
            array('id' => 7, "name" => "Taxes", "key" => "new_account_taxes"),
            array('id' => 8, "name" => "Currency ", "key" => "new_account_currency"),
            array('id' => 9, "name" => "Payment Mode", "key" => "new_account_payment_mode"),
            array('id' => 10, "name" => "Suppliers", "key" => "new_account_suppliers"),
            array('id' => 11, "name" => "Bancs", "key" => "new_account_bancs"),
            array('id' => 12, "name" => "Products", "key" => "new_account_products"),
            array('id' => 13, "name" => "Immobilitzed", "key" => "new_account_immobilitzed"),
            array('id' => 14, "name" => "Staff", "key" => "new_account_staff"),
        );
        $data['document_module'] = array(
            array('name' => 'Main Options', 'options' => array(
                array('id' => 1, "name" => "Invoices", "key" => "document_invoices"),
                array('id' => 2, "name" => "Expenses", "key" => "document_expenses"),
                array('id' => 3, "name" => "Payrolls", "key" => "document_payrolls"),
            )),
            array('name' => 'Purchase', 'options' => array(
                array('id' => 4, "name" => "Purchase Order", "key" => "document_purchase_order"),
                array('id' => 5, "name" => "Purchase Payment", "key" => "document_purchase_payment"),
                array('id' => 5, "name" => "Purchase Invoice", "key" => "document_purchase_invoice"),
                array('id' => 5, "name" => "Purchase document", "key" => "document_purchase_document"),
            )),
            array('name' => 'Inventory', 'options' => array(
                array('id' => 6, "name" => "Receiving Boucher", "key" => "document_receiving_boucher"),
                array('id' => 7, "name" => "Delivery", "key" => "document_delivery"),
                array('id' => 8, "name" => "Loss", "key" => "document_loss"),
                array('id' => 9, "name" => "Adjustment", "key" => "document_adjustment"),
                array('id' => 10, "name" => "Opening Stock", "key" => "document_opening_stock"),
            )),
            array('name' => 'Payslips', 'options' => array(
                array('id' => 11, "name" => "Received", "key" => "document_received"),
                array('id' => 12, "name" => "Done", "key" => "document_done"),
            ))
        );
        $data['d_or_c'] = array(
            array('id' => 1, "name" => "Debit" , "key" => "debit"),
            array('id' => 2, "name" => "Credit", "key" => "credit"),
        );
        $data['mapping_rule'] = $this->accounting_model->get_mapping();
        $data['mapping_settlement'] = $this->accounting_model->mapping_settlement();
    ...
    /**
     * get relation variable
     */
    public function get_relation_variable(){
        if (isset($_GET['data'])) {
        $data = $_GET['data'];
        if ($data == "document_invoices" || $data == "unified_invoices") {
            $table_name = "invoices";
        } else if ($data == "document_expenses" || $data == "unified_expenses") {
            $table_name = "expenses";
        } else if ($data == "document_payrolls" || $data == "unified_payrolls") {
            $table_name = "payroll_table";
        } else if ($data == "document_purchase_order" || $data == "document_purchase_payment" || $data == "document_purchase_invoice" || $data == "document_purchase_document" ||
        $data == "unified_purchase_order" || $data == "unified_purchase_payment" || $data == "unified_purchase_invoice" || $data == "unified_purchase_document"
        ) {
            $table_name = "product_purchase_log";
        } else if (
            $data == "document_receiving_boucher" || 
            $data == "document_delivery" || 
            $data == "document_loss" || 
            $data == "document_adjustment" || 
            $data == "document_opening_stock" ||
            $data == "unified_receiving_boucher" || 
            $data == "unified_delivery" || 
            $data == "unified_loss" || 
            $data == "unified_adjustment" || 
            $data == "unified_opening_stock"
        ) {
            $table_name = "inventory_manage";
        } else if ($data == "document_received" || $data == "document_done" || $data == "unified_done" || $data == "unified_received") {
            $table_name = "hrp_payslips";
        }else if ($data == "unified_custom_fields") {
            $table_name = "customfields";
        }

        if ($table_name == "customfields") {
            $this->db->select('slug');
            $column_names = $this->db->get(db_prefix() . 'customfields')->result_array();
            $column_names = array_column($column_names, 'slug');
        }else{
            $this->load->database();
            $table_name = db_prefix() . $table_name;
            $column_names = $this->db->list_fields($table_name);
        }
        echo json_encode(['data' => $column_names, 'message' => "added_successfully"]);die();
        } else {
        echo json_encode(['error' => 'Data parameter is missing']);die();
        }
    }
    ...
    public function account()
    {
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
          	unset($data['proposal_text_custom']);
            unset($data['proposal_number_custom']);
            unset($data['proposal_textarea_custom']);
            unset($data['proposal_dropdown_custom']);
            unset($data['proposal_checkbox_custom']);
            unset($data['proposal_radio_custom']);
            unset($data['proposal_date_custom']);
            unset($data['proposal_file_custom']);
     ...
     * @return view
     */
    public function unified_book(){
        if (!has_permission('accounting_journal_entry', '', 'view')) {
            access_denied('accounting');
        }
        $data['group'] = $this->input->get('group');
        $data['title']         = _l('unified_book');
        $data['tab'][] = 'unified_book';
        $data['tab'][] = 'configuration';
        $data['get_unified_book'] = $this->accounting_model->get_unified_book();
        $data['get_unified_row_mapping'] = $this->accounting_model->get_unified_row_mapping();
        $data['accounts_to_select'] = $this->accounting_model->get_data_account_to_select();
        $data['unified_book_mapping'] = $this->accounting_model->unified_book_mapping();
        $data['document_module'] = array(
            array('name' => 'Main Options', 'options' => array(
                array('id' => 1, "name" => "Invoices", "key" => "unified_invoices"),
                array('id' => 2, "name" => "Expenses", "key" => "unified_expenses"),
                array('id' => 3, "name" => "Payrolls", "key" => "unified_payrolls"),
            )),
            array('name' => 'Purchase', 'options' => array(
                array('id' => 4, "name" => "Purchase Order", "key" => "unified_purchase_order"),
                array('id' => 5, "name" => "Purchase Payment", "key" => "unified_purchase_payment"),
                array('id' => 6, "name" => "Purchase Invoice", "key" => "unified_purchase_invoice"),
                array('id' => 7, "name" => "Purchase document", "key" => "unified_purchase_document"),
            )),
            array('name' => 'Inventory', 'options' => array(
                array('id' => 8, "name" => "Receiving Boucher", "key" => "unified_receiving_boucher"),
                array('id' => 9, "name" => "Delivery", "key" => "unified_delivery"),
                array('id' => 10, "name" => "Loss", "key" => "unified_loss"),
                array('id' => 11, "name" => "Adjustment", "key" => "unified_adjustment"),
                array('id' => 12, "name" => "Opening Stock", "key" => "unified_opening_stock"),
            )),
            array('name' => 'Payslips', 'options' => array(
                array('id' => 13, "name" => "Received", "key" => "unified_received"),
                array('id' => 14, "name" => "Done", "key" => "unified_done"),
            )),
            array('name' => 'Settings', 'options' => array(
                array('id' => 15, "name" => "General", "key" => "unified_general"),
                array('id' => 16, "name" => "Company Information", "key" => "unified_company"),
                array('id' => 17, "name" => "Customers", "key" => "unified_customers")
            )),
            array('name' => 'Custom Fields', 'options' => array(
                array('id' => 18, "name" => "Custom Fields", "key" => "unified_custom_fields")
            ))
        );
        if($data['group'] == 'unified_book'){
            $this->load->view('unified_book/manage', $data);
        }else if ($data['group'] == 'configuration') {
            $this->load->view('unified_book/configuration', $data);
        }
    }
     ...
     * update unified book
     */
    public function update_unified_book(){
        if (!has_permission('accounting_setting', '', 'edit') && !is_admin()) {
            access_denied('accounting_setting');
        }
        $data = $this->input->post();
        $success = $this->accounting_model->update_unified_book($data);
        if($success == true){
            $message = _l('updated_successfully', _l('setting'));
            set_alert('success', $message);
        }
        redirect(admin_url('accounting/unified_book?group=configuration'));
    }
    ...
    public function journal_entry_table(){
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                '1', // bulk actions
                db_prefix() . 'acc_journal_entries.id',
                'number',
                'journal_date',
            ];
            ...
            $join         = ['LEFT JOIN ' . db_prefix() . 'acc_account_history ON ' . db_prefix() . 'acc_journal_entries.id = ' . db_prefix() . 'acc_account_history.rel_id'];
            $result       = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
                db_prefix() . 'acc_journal_entries.amount',
                db_prefix() . 'acc_journal_entries.description',
                db_prefix() . 'acc_account_history.settlement',
                db_prefix() . 'acc_account_history.line',
                db_prefix() . 'acc_account_history.document',
                db_prefix() . 'acc_account_history.debit',
                db_prefix() . 'acc_account_history.credit',
                db_prefix() . 'acc_account_history.balance',
                db_prefix() . 'acc_account_history.account_name',
                db_prefix() . 'acc_account_history.account',
                db_prefix() . 'acc_account_history.approved']);
            ...
            foreach ($rResult as $aRow) {
                $row   = [];
                $row[] = '<div class="checkbox"><input type="checkbox" value="' . $aRow['tblacc_journal_entries.id'] . '"><label></label></div>';
                $categoryOutput = _d($aRow['journal_date']);

                $categoryOutput .= '<div class="row-options">';

                if (has_permission('accounting_journal_entry', '', 'edit')) {
                    $categoryOutput .= '<a href="' . admin_url('accounting/journal_entry_export/' . $aRow['tblacc_journal_entries.id']) . '" class="text-success">' . _l('acc_export_excel') . '</a>';
                }

                if (has_permission('accounting_journal_entry', '', 'edit')) {
                    $categoryOutput .= ' | <a href="' . admin_url('accounting/new_journal_entry/' . $aRow['tblacc_journal_entries.id']) . '">' . _l('edit') . '</a>';
                }

                if (has_permission('accounting_journal_entry', '', 'delete')) {
                    $categoryOutput .= ' | <a href="' . admin_url('accounting/delete_journal_entry/' . $aRow['tblacc_journal_entries.id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
                }
     ...
     * @return json
     */
    public function journal_unified_book(){
        if ($this->input->is_ajax_request()) {
            ...
            $columns = $this->db->list_fields(db_prefix() . 'unified_book');
            $select = array_merge($select, $columns);
            $Fields_For_Loop = array_filter($columns, function ($column) {
                return $column != 'id';
            });
            ...
            $aColumns     = $select;
            $sIndexColumn = 'id';
            $sTable       = db_prefix() . 'unified_book';          
            $join         = [];

            $additionalFields = array_filter(array_map(function ($column) {
                if ($column != "id") {
                    return db_prefix() . 'unified_book.' . $column;
                }
            }, $columns));
            $result       = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, $additionalFields);
            $output  = $result['output'];
            $rResult = $result['rResult'];
            foreach ($rResult as $aRow) {
                $row   = [];
                $row[] = '<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>';
                $categoryOutput = $aRow['id'];

                $categoryOutput .= '<div class="row-options">';

                if (has_permission('accounting_journal_entry', '', 'edit')) {
                    $categoryOutput .= '<a href="' . admin_url('accounting/unified_entry_export/' . $aRow['id']) . '" class="text-success">' . _l('acc_export_excel') . '</a>';
                }

                if (has_permission('accounting_journal_entry', '', 'edit')) {
                    $categoryOutput .= ' | <a href="' . admin_url('accounting/edit_unifiedbook_entry/' . $aRow['id']) . '">' . _l('edit') . '</a>';
                }

                if (has_permission('accounting_journal_entry', '', 'delete')) {
                    $categoryOutput .= ' | <a href="' . admin_url('accounting/journal_entry/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
                }

                $categoryOutput .= '</div>';
                $row[] = $categoryOutput;
                $this->db->select('value');
                $this->db->from(db_prefix() .'unified_book_mapping');
                $this->db->order_by('row_number ASC');
                $unified_book_mapping = $this->db->get()->result_array();
                foreach ($unified_book_mapping as $column) {
                $row[] = $aRow[$column['value']];
                }
                // if(strlen($aRow['number'].' - '.html_entity_decode($aRow['description'])) > 150){
                //     $row[] = '<div data-toggle="tooltip" data-title="'. $aRow['number'].' - '.html_entity_decode(strip_tags($aRow['description'])).'">'.substr($aRow['number'].' - '.html_entity_decode($aRow['description']), 0, 150).'...</div>';
                // }else{
    ...
    public function new_journal_entry($id = ''){
        ...
        $data['title'] = _l('journal_entry');
        $data['account_to_select'] = $this->accounting_model->get_data_account_to_select();
        $data['account_to_select_number'] = $this->accounting_model->get_data_account_to_select_number();
     ...
     * @param  integer $id
     */
    public function unified_entry_export($id){
        ...
        // $header = [ _l('asp_order'), _l('asp_date'), _l('asp_creation_date'), _l('asp_invoice_number'), _l('asp_reference'), _l('asp_book'), _l('asp_account'), _l('asp_nif'), _l('asp_desc'), _l('asp_total_invoice'), _l('asp_subtotal_1'), _l('asp_vat_1'), _l('asp_subtotal_2'), _l('asp_vat_2'), _l('asp_subtotal_3'), _l('asp_vat_3'),  _l('asp_subtotal_4'), _l('asp_vat_4'),  _l('asp_subtotal_5'), _l('asp_vat_5'), _l('asp_libro_contrapartida'), _l('asp_cuenta_contrapartida'), _l('asp_lote_a_contabilizar')];
            ...
            $journal_entry = $this->accounting_model->get_unified_entry($id);
    ...
    // $header = array_merge($header, $columns);

// Dynamic generation of data
$data = [];
    $rowData = [];
foreach ($journal_entry as $k => $row) {
    // print_r($row);exit;
    ...
    // Additional data for unified book
    // $rowData[] = isset($account_name[$row['account']]) ? $account_name[$row['account']] : $row['account'];
    // $rowData[] = $row['debit'] > 0 ? app_format_money($row['debit'], $currency->name) : '';
    // $rowData[] = $row['credit'] > 0 ? app_format_money($row['credit'], $currency->name) : '';

        // $rowData[] = $row['clientid'];
...
//         $writer = new XLSXWriter();
//         $writer->writeSheetRow('Sheet1', []);
//         $writer->writeSheetRow('Sheet1', [1 => _l('clientid').': '. _d($journal_entry->journal_date), ]);
//         $writer->writeSheetRow('Sheet1', [1 => _l('debit').': '. $journal_entry->number, ]);
//         $writer->writeSheetRow('Sheet1', [1 => _l('description').': '. $journal_entry->Description, ]);
...
//         }
        ...
        $writer->writeSheetRow('Sheet1', [], $style3);
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        for ($row = 1; $row < count($data); $row++) {
                            ...
                            $result = $this->db->select('order')->from(db_prefix().'acc_account_types')->order_by('order', 'DESC')->limit(1)->get()->result();
                            $newOrder = $result[0]->order+1;
                            $InsertData = array(
                                'name' => $value_type,
                                'order'=> $newOrder
                            );
                            $this->db->insert(db_prefix().'acc_account_types', $InsertData);
                            $last_inserted_id = $this->db->insert_id();

                            $InsertAccountTypeDetails = array(
                                'account_type_id' => $last_inserted_id,
                                'name'=> $value_sub_type
                            );
                            $this->db->insert(db_prefix().'acc_account_type_details', $InsertAccountTypeDetails);
                            }
    ...
    /**
     * remove mapping setting
     */
    public function delete_mapping($mapping_id){
        if ($mapping_id) {
            $result = $this->accounting_model->delete_mapping($mapping_id);
            if($result == true){
            $message = _l('updated_successfully', _l('setting'));
            set_alert('success', $message);
        }
     ...
     /**
     * remove unified Book mapping setting
     */
     
     public function delete_unified_mapping($mapping){
       
         if($mapping){
             $success = $this->accounting_model->delete_unified_mapping($mapping);
        
            if($success == true){
            $message = _l('delete_successfully', _l('setting'));
            set_alert('success', $message);
        }
         }
        redirect(admin_url('accounting/accounting/unified_book?group=configuration'));
    }
    
    
    /**
     * Edit unified Book entry 
    */
   public function edit_unifiedbook_entry($id = ''){
        ...
        if ($this->input->post()) {
            ...
            if($id == ''){
                ...
                if ($success === 'close_the_book'){
                    ...
                    set_alert('success', _l('added_successfully', _l('unified_book')));
                }
            }else{
                if (!has_permission('accounting_journal_entry', '', 'edit')) {
                    access_denied('accounting_journal_entry');
                }
                $success = $this->accounting_model->update_unified_book_data($data, $id);
                if ($success === 'close_the_book') {
                    $message = _l('has_closed_the_book');
                    set_alert('warning', _l('has_closed_the_book'));
                }elseif ($success) {
                    set_alert('success', _l('updated_successfully', _l('unified_book')));
                }
            }
            redirect(admin_url('accounting/unified_book?group=unified_book'));
        }
        if($id != ''){
            $data['unified_book'] = $this->accounting_model->get_unified_entry($id);
        }
        $data['title'] = _l('unified_book');
        $this->load->view('unified_book/journal_entry', $data);
    }
}