...
<div id="setup-menu-wrapper" class="sidebar animated <?php if ($this->session->has_userdata('setup-menu-open')
    ...
    <ul class="nav metis-menu" id="setup-menu">
        ...
        foreach ($setup_menu as $key => $item) {
            if ($this->app_modules->is_active('saas')) {
                if (IS_TENANT && $item['slug'] == 'modules') {
                    continue;
                }
            }
            ...
            $totalSetupMenuItems++; ?>
        <li class="menu-item-<?php echo $item['slug']; ?>" data-toggle="tooltip" data-placement="right" title="<?php echo $item['tooltip'] ?? "" ?>">
            ...
            <ul class="nav nav-second-level collapse" aria-expanded="false">
                <?php foreach ($item['children'] as $submenu) {
                            ?>
                <li class="sub-menu-item-<?php echo $submenu['slug']; ?>" data-toggle="tooltip" data-placement="right" title="<?php echo $submenu['tooltip'] ?? "" ?>"><a href="<?php echo $submenu['href']; ?>">
