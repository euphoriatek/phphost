<script type="text/javascript">
    ...
    if (dataToImport) {
    var load_action = dataToImport.drawflow.Home.data;
      for (const key in load_action) {
        if (load_action.hasOwnProperty(key)) {
          if(load_action[key]['name'] == "flow_start"){
              load_actions(load_action[key]['data'].flows);
            }
        }
      }
    }
   ...
   function allowDrop(ev) {
    ...
    function addNodeToDrawFlow(name, pos_x, pos_y) {
      ...
      switch (name) {
        case 'flow_start':
            $.post(admin_url + 'acf_flows/get_workflow_node_html', {
                type: 'flow_start',
                nodeId: editor.nodeId,
                csrf_token_name: $('input[name=csrf_token_hash]').val(),
                url_type: new URL(window.location.href).searchParams.get("type")
            }).done(function (html) {
  ...
  function showpopup(e) {
   ...
   function closemodal(e) {
    ...
    function load_actions(value){
      "use strict";
          if (value) {
          var actionsDropdown = $('#actions_dropdown');
          actionsDropdown.empty();
          $.ajax({
          type: 'GET',
          url: 'http://localhost/accountcrm/perfex_crm/admin/acf_flows/get_actions',
          data:{flow_id:value},
